...
Les modules sont triés par onglet. Les modules qui ne sont dans aucun onglet seront placés dans la section **Orphaned Modules**, cela signifie généralement qu'il y a un problème avec le module, mais ce n'est pas toujours le cas.
...
Un simple toggle local d'objets. Utilisé par l'onglet de paramètres par défaut.

- ToggleOnObjects: GameObject[] - Liste des objets qui seront activés quand l'état du toggle est True.
- ToggleOffObjects: GameObject[] - Liste des objets qui seront désactivés quand l'état du toggle est True.
- Button: Button - Référence au bouton utilisé pour basculer, utilisée pour mettre à jour la coche.
- Checkmark: Image - Référence à l'image de coche, utilisée pour l'afficher ou la cacher selon l'état du toggle.
- State: bool - Valeur par défaut du toggle.
- DisableOnQuest - Si true, l'interactabilité de ce toggle sera désactivée sur les plateformes android et ios.
...
- Text : TextMeshProUGUI - Composant texte utilisé pour récupérer le message d'avertissement.
